...
import { floorsData, description } from '../../data/TowersData';
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden flex justify-center items-center">
      ...
      {/* <div className='w-full h-full bg-red-500 '>
        <svg viewBox="0 0 3000 1828" preserveAspectRatio="xMidYMid meet" className='w-full h-full'>
          <image href={bgImage} x="0" y="0" width="3000" height="1828" />
          {floorsData.map((floor) => (
            ...
            <polygon
              ...
              // width={floor.width}
              // height={floor.height}
              fill={hoveredId === floor.id ? floor.hoverColor : "transparent"}
              cursor="pointer"
              data-tooltip-id={`tooltip-${floor.id}`}
              data-tooltip-content={floor.name}
              onMouseEnter={() => setHoveredId(floor.id)}
              onMouseLeave={() => setHoveredId(null)}
              onDoubleClick={() =>
                navigate(`/arena_floorpavilion/${floor.id}`)
              }
            />
